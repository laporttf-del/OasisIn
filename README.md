...
body{
  ...
  background:#0b2817; /* verde escuro */
  color:#ffffff;
...
button{
  background:#00aa55;cursor:pointer;font-weight:600;margin-top:10px;
  border:none;
}
...
th{color:#00ff99;}
td.gasto{color:#ff5555;font-weight:600;} /* vermelho nos gastos */
...
function atualizarUI(filtroMes=null,filtroAno=null){
  ...
  filtrados.forEach(r=>{
    ...
    const expected=r.litrosHora*horas;
    const nota=calcularNota(expected,r.usado);
  ...
  // Gráfico de pizza com gastos dos operadores
    ...
    grafico=new Chart(ctx,{
      ...
      data:{
        ...
        datasets:[{
          data:valores,
          backgroundColor:[
            '#00ff99','#00995c','#33cc99','#ff5555','#ffaa00','#3366ff','#00ccff'
          ]
        }]
      },
      options:{
...
document.getElementById('form').addEventListener('submit',e=>{
  ...
  registros.push({data,operador:op,maquina:maq,horaLigou:hL,horaDesligou:hD,dieselInicial:ini,dieselFinal:fin,litrosHora:lh,usado,gasto});
  salvar(registros);
  e.target.reset();
  atualizarUI();
});
